    ...
    if (isset($_SESSION["Usuario"])){
       if($_SESSION["Permisos"][6] == TRUE){ 
            ...
            if(isset($_POST['NombreCat'])){
                ...
                $Id_categoria = $_GET['Id_trabajo'];
                    $resultado = eliminar_categoria($Id_categoria);
                   if($resultado == TRUE){
                    echo '<div id="notify" class="alert alert-success" role="alert">
                        ¡EL servicio ha sido eliminado de manera exitosa!
                        </div>';
                    echo '<script>
                      setTimeout(function(){$("#notify").remove();}, 3000);
                  </script>';
                ...
                include('../html/Ajustes/Servicios/_consultar_servicio.html');

            } else  if( isset($_GET['Id_trabajo']) && isset($_GET['editar']) && $_GET['editar']==1){
                $Id_categoria = $_GET['Id_trabajo'];
                $disabled = 0;
                $data =consultar_cat_ser($Id_categoria);
                include('../html/Ajustes/Servicios/_modificar_servicios.html');
            } else if(isset($_GET['Id_trabajo'])){
                $Id_categoria = $_GET['Id_trabajo'];
                $disabled = 1;
                $data = consultar_cat_ser($Id_categoria);
                include('../html/Ajustes/Servicios/_modificar_servicios.html');
